...
module.exports = function command (yargs, usage, validation, globalMiddleware) {
  ...
  self.runCommand = function runCommand (command, yargs, parsed, commandIndex) {
    ...
    let preCheckMiddlewares = globalMiddleware.slice(0).concat(commandHandler.middlewares || []).filter((x) => x.applyBeforeValidation === true)
    if (preCheckMiddlewares.length > 0 && !yargs._hasOutput()) {
      ...
      const middlewareArgs = preCheckMiddlewares.reduce(function (initialObj, middleware) {
        ...
        let result = middleware(innerArgv, preChecksContext)
