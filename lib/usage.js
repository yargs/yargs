...
module.exports = function usage (yargs, y18n) {
  ...
  self.help = function help () {
    ...
    if (!usageDisabled) {
      ...
      if (usages.length) {
        // user-defined usage.
        u = self.getUsage().replace(/\$0/g, path.basename(yargs.$0))
      } else if (commands.length) {
        // demonstrate how commands are used.
        if (demandedCommands._) {
          u = `${yargs.$0} <${__('command')}>\n`
        } else {
          u = `${yargs.$0} [${__('command')}]\n`
        }
      }
    ...
    if (commands.length) {
      ...
      commands.forEach((command) => {
        const commandString = `${yargs.$0} ${command[0].replace(/^\$0 ?/, '')}` // drop $0 from default commands.
        ui.span(
          {
            text: commandString,
            padding: [0, 2, 0, 2],
            width: maxWidth(commands, theWrap, yargs.$0) + 4
          },
          {text: command[1]}
        )
        const hints = []
