...
module.exports = function (yargs, usage, y18n) {
  ...
  self.nonOptionCount = function (argv) {
    ...
    if (demanded._ && (_s < demanded._.count || _s > demanded._.max)) {
      if (typeof demanded._.msg === 'string') {
        usage.fail(demanded._.msg.replace(/\$0/g, argv._.length).replace(/\$1/g, demanded._.count))
      } else if (demanded._.msg !== undefined) {
        usage.fail(demanded._.msg)
      } else if (_s < demanded._.count) {
        usage.fail(
          __('Not enough non-option arguments: got %s, need at least %s', _s, demanded._.count)
        )
      } else if (typeof demanded._.maxMsg === 'string') {
        usage.fail(
          demanded._.maxMsg.replace(/\$0/g, argv._.length).replace(/\$1/g, demanded._.count)
        )
      } else {
        usage.fail(
