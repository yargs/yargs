...
async function applyMiddleware (argv, yargs, middlewares, beforeValidation) {
  for (let middleware of middlewares.filter(m => m.applyBeforeValidation === beforeValidation)) {
    Object.assign(argv,
      middleware.length < 3
        ? await middleware(argv, yargs)
        // middleware using a done callback
        : await new Promise(resolve => middleware(argv, yargs, resolve))
    )
    // TODO: add a test for middleware calling done
  }
}
