...
module.exports.applyPreCheckMiddlware = function (argv, middlewares, context, aliases) {
  for (let i = 0; i < middlewares.length; i++) {
    let preChecksContext = {
      'commands': JSON.parse(JSON.stringify(context.commands)),
      'availableOptions': JSON.parse(JSON.stringify(aliases))
    }
    let result = middlewares[i](argv, preChecksContext)
