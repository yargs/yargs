...
function Yargs(processArgs, cwd, parentRequire) {
  ...
  if (
    ...
    processArgv.getProcessArgvBin() === process.env._
  ) {
  ...
  self.resetOptions = self.reset = function resetOptions(aliases) {
    ...
    validation = validation
      ...
      : Validation(self, usage, y18n);
    command = command
      ? command.reset()
      : Command(self, usage, validation, globalMiddleware);
  ...
  self.config = function config(key, msg, parseFn) {
    ...
    if (typeof key === 'object') {
      key = applyExtends(
        key,
        cwd,
        self.getParserConfiguration()['deep-merge-config']
      );
  ...
  function pkgUp(rootPath) {
    ...
    try {
      let startDir =
        rootPath || require('require-main-filename')(parentRequire || require);
  ...
  self.terminalWidth = () => {
    argsert([], 0);
    return typeof process.stdout.columns !== 'undefined'
  ...
  self._parseArgs = function parseArgs(
    ...
    try {
      ...
      if (shortCircuit) {
        return populateDoubleDash || _calledFromCommand
          ? argv
          : self._copyDoubleDash(argv);
      }
          ...
          for (let i = commandIndex || 0, cmd; argv._[i] !== undefined; i++) {
            ...
            if (~handlerKeys.indexOf(cmd) && cmd !== completionCommand) {
              ...
              return populateDoubleDash
                ? innerArgv
                : self._copyDoubleDash(innerArgv);
