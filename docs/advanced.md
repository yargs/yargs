...
Middleware can be added (and processed) prior to any validation checks but after parsing so required fields maybe implicitly obtained from other methods (such as searching config files, looking for specific files in the current working directory), etc. These can only be set globally at the moment.
